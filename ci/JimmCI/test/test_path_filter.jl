...
@testset "families_for_paths" begin
    ...
    @testset "shared prefixes promote to all families" begin
        ...
        @test families_for_paths(["src/Models/ConvNeXtCommon/x.jl"]) == collect(ALL_FAMILIES)
    ...
    @testset "multi-family in canonical order" begin
        @test families_for_paths([
            "src/Models/ConvNeXt/x.jl",
            "src/Models/ResNet/y.jl",
        ]) == ["resnet", "convnext"]
    end

    @testset "shared wins over per-family" begin
        @test families_for_paths([
            "src/Models/ResNet/foo.jl",
            "src/Layers/conv.jl",
